...
import { createBooking, listParties, listProjects, type PartyRow, type ProjectRow } from '@/db';
...
/**
 * Book material ahead ("5000 bricks @ Rs 10"). On the shared AppSheet, with the
 * shared MaterialItemPicker (carries the unit) + QtyUnitRow (secondary-unit
 * aware). Supplier is one control: a name field with saved-supplier quick-fill.
 */
export function NewBookingSheet({ visible, onClose, onSaved }: Props): React.JSX.Element {
  ...
  return (
    ...
    >
      ...
      {/* Supplier — one control: a name field + saved-supplier quick-fill chips. */}
      <FloatingLabelInput
        label={t('supplier')}
